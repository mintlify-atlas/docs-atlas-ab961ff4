...
sidebarTitle: "KKT"
description: "Solve constrained nonlinear programs using Karush-Kuhn-Tucker conditions. Enumerates active constraint combinations, solves each KKT system, and returns the global optimum."
---
...
<ResponseField name="cases" type="KKTCase[]">
  ...
  <Expandable title="KKTCase fields">
    ...
    <ResponseField name="lambdas" type="object | null">
      Dictionary of inequality multipliers, keyed as `"λ1"`, `"λ2"`, etc., indexed over active constraints. `null` for `"no_convergence"` cases.
    </ResponseField>
...
<Note>
  `lambdas` (`λ`) are the **inequality constraint multipliers**. For a valid KKT point, all λᵢ must be non-negative (dual feasibility). They are keyed by their position among the active constraints in the enumerated subset, not by their original index in the `constraints` array.
